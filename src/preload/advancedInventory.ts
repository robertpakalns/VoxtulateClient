...
const advancedInventory = async () => {
  ...
  const _inventoryButton = createEl("div", {}, "voxiomSkinsButton", [
    "Advanced Sorting",
  ]);
  ...
  const observer = new MutationObserver(() => {
    ...
    const isMarket =
      pathname === "/loadouts/market" ||
      (pathname === "/loadouts" && document.querySelector(".dzKqCZ"));
    const isSales = pathname === "/loadouts/sales";

    if (
      inmenu.data?.data?.length > 0 &&
    ...
    for (const [i, el] of Array.from(
      ...
      const _image =
        isMarket || isSales ? createEl("img", { src: gemPath }, "gem") : "";
      const text =
        isSales || isMarket
          ? `${timeLeft(skin.listed_time + 1209600000)} | ${skin.price}`
          : "";
      const _name = createEl("div", { textContent: text }, "voxiomSkinName", [
        _image,
      ]);
